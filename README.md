...
1. **Provision (Omni).** Apply the cluster template `omni/cluster-template/cluster.yaml` — Talos v1.13.8 / Kubernetes v1.36.3, 3 control-plane nodes (from the `worker` machine class, with `allowSchedulingOnControlPlanes`) in `omni/machineclass/`. The `omni/patches/cluster-config.yaml` patch disables kube-proxy and the built-in CNI, adds the Longhorn disk/mounts, exposes the control-plane metrics endpoints (etcd/scheduler/controller-manager on `0.0.0.0`), and adds bootstrap `extraManifests` (kubelet-serving-cert-approver, metrics-server, Gateway API CRDs).
